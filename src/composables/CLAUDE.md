...
集中管理应用全局状态与所有业务逻辑。`App.vue` 调用 `useAppState()` 后通过 `provide('appState', ...)` 注入，所有 Step 组件通过 `inject('appState')` 获取。
...
└── ui                          — { activeTab, activeSteps: number[], processing, conflictSearch }
```
...
| `runMerge()` | Step4 "开始合并" | async；设置 `processing=true`，调用 `combineSheetData` + `classifyMerge`，写入 `mergeResult`，启用 Step5 |
| `resolveConflict(ci, action)` | Step5 单个冲突 | 按数字索引 `ci` 写入 `conflictResolutions[key]` |
| `resolveAllConflicts(action)` | Step5 批量操作 | 批量调用 `resolveConflict` |
| `downloadExcel()` | Step5 / App.vue footer | 调用 `buildFinalOutput`，按 outputOptions 构建多 sheet XLSX |
| `downloadCSV()` | Step5 | 调用 `buildFinalOutput`，转为带 BOM 的 CSV Blob 下载 |
